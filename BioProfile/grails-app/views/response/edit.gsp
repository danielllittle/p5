...
<html>
	...
	<body>
		...
		<div id="edit-response" class="content scaffold-edit" role="main">
			...
			<g:hasErrors bean="${response}">
			<ul class="errors" role="alert">
				<g:eachError bean="${response}" var="error">
				<li <g:if test="${error in org.springframework.validation.FieldError}">data-field-id="${error.field}"</g:if>><g:message error="${error}"/></li>
				</g:eachError>
			</ul>
			</g:hasErrors>
			<g:form url="[resource:response, action:'update']" method="PUT" >
				<g:hiddenField name="version" value="${response?.version}" />
				<fieldset class="form">
